...
#Solution:
def maskPersonalInformation(s: str) -> str:
    if '@' in s:  # Email case
        name, domain = s.split('@')
        name = name.lower()
        masked_name = name[0] + '*****' + name[-1] if len(name) > 2 else name.lower()
        ...
        #Space Complexity: O(n), for storing the filtered digits and the masked output.
